    ...
    guint64 clockCompensation)
{
  ...
  /* when calculating clock compensation, adaptive demux will add half of
   * the round trip delay between the client and the server.
   * Let's add 500ms to take that into account, otherwise we might compute
   * a time smaller than what adaptive demux has computed and returned in the
   * seek range.
   */
    ...
    gpointer user_data)
{
  ...
  if (strstr (srcName, "bin") == srcName &&
    ...
    pads = GST_ELEMENT_PADS (msg->src);
    fail_unless (pads != NULL);
    fail_unless (g_list_length (pads) == 1);

    internal_pad = gst_pad_get_peer (GST_PAD (pads->data));

    /* stream pads are ghost pads so we can get them from their internal pad */
    stream_pad =
        GST_PAD_CAST (gst_proxy_pad_get_internal (GST_PROXY_PAD
            (internal_pad)));
    stream_pad_name = gst_pad_get_name (stream_pad);
    fail_unless (stream_pad_name != NULL);
    gst_object_unref (internal_pad);
    gst_object_unref (stream_pad);

    if (strcmp (stream_pad_name, "audio_00") == 0) {
      g_mutex_lock (&testData->test_task_state_lock);
    ...
    GstEvent * event, gpointer user_data)
{
  ...
  if (GST_EVENT_TYPE (event) == GST_EVENT_PROTECTION) {
    ...
    fail_unless (strcmp (name, "video_00") == 0);
    g_free (name);
    ...
    if (strcmp (system_id, "11111111-aaaa-bbbb-cccc-123456789abc") == 0) {
      fail_unless (strcmp (origin, "dash/mpd") == 0);
      fail_unless (strcmp (value, "test value") == 0);
    } else if (strcmp (system_id, "5e629af5-38da-4063-8977-97ffbd9902d4") == 0) {
      const gchar *str;
